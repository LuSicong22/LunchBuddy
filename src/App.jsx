...
export default function LunchBuddyApp() {
  ...
  const hasDismissedInstallPrompt = () =>
    ...
    localStorage.getItem(installPromptDismissedKey) === "1";
  ...
  useEffect(() => {
    ...
    const updateDisplayMode = () =>
      setIsStandalone(mediaQuery.matches || window.navigator.standalone);
  ...
  const handlePartnerAccept = () => {
    ...
    setFriends((prev) =>
      prev.map((f) =>
        f.id === friendToDate.id ? { ...f, status: "inactive" } : f
      )
    );
    setFriendToDate(null);
    setDatingStep("confirm");
  ...
  const isGroupDining = (input) => {
    ...
    const sizeText =
      typeof input === "string" ? input : input.sizePreference || input.size;
    const maxSize = parseMaxSize(sizeText);
    if (maxSize) return maxSize > 2;
    if (typeof input === "object" && input.participants)
      return input.participants.length > 2;
    return false;
  };
  ...
  const getParticipantProfile = (participant) =>
  ...
  const handleJoinOpenEvent = (event) => {
    ...
    const joinedEvent = {
      ...event,
      joined: true,
      participants: [...event.participants, selfParticipant],
    };

    setOpenDiningEvents((prev) =>
      prev.map((item) => (item.id === event.id ? joinedEvent : item))
    );
      ...
      <div className="bg-gradient-to-br from-orange-50 to-white rounded-2xl p-4 shadow-md border border-orange-100 animate-slide-up relative overflow-hidden mb-4">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center">
              ...
              {participants.map((p, idx) => (
                <div
                  ...
                  className={`w-10 h-10 rounded-full ${
                    p.avatarColor
                  } border-2 border-white flex items-center justify-center text-white text-xs font-bold shadow-sm z-[${
                    10 - idx
                  }]`}
                >
                  {p.nickname[0]}
            ...
            <div className="ml-3">
              ...
              <span className="text-xs text-gray-500 block">
                等 {participants.length} 人
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap mt-1">
            <span className="bg-orange-100 text-orange-700 text-xs px-2 py-0.5 rounded-md font-medium">
              {event.food}
            </span>
          ...
          >
            {event.joined ? (
              <>
                <Check size={14} /> 已加入{eventLabel}
              </>
            ) : (
              <>
                <LogIn size={14} /> 加入{eventLabel}
              </>
            )}
          </button>
        </div>
  ...
  const HomeView = () => {
    const visibleOpenEvents = openDiningEvents.filter((event) =>
      eventHasFriend(event)
    );
    const activeFriends = friends.filter((f) => f.status === "active");
    const activeItems = [
    ...
    const renderActiveItem = (item) =>
      item.type === "event" ? (
        <OpenDiningCard key={item.id} event={item.data} />
      ) : (
        <FriendCard key={item.id} friend={item.data} />
      );
    if (confirmedDining) {
      return (
              ...
              <div className="pt-12 pb-6 px-6 text-center space-y-5">
                {confirmedDining.isGroup ? (
                  <div className="space-y-3">
                    ...
                    <p className="text-gray-800 font-bold text-lg truncate px-4">
                      {(confirmedDining.participants || [])
                        .map((p) => p.nickname)
                        .join(" & ")}
                    </p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-orange-50 p-3 rounded-xl">
                    ...
                    <p className="font-bold text-gray-800">
                      {confirmedDining.isGroup
                        ...
                        : confirmedDining.partner.lunchPlan?.hideFood &&
                          diningViewMode === "me"
                        ? "🤫 秘密"
                        : confirmedDining.food}
                    </p>
                  ...
                  <div className="bg-purple-50 p-3 rounded-xl">
                    ...
                    <p className="font-bold text-gray-800">
                      {confirmedDining.isGroup
                        ...
                        : confirmedDining.partner.lunchPlan?.hideLocation &&
                          diningViewMode === "me"
                        ? "🤫 秘密"
                        : confirmedDining.location}
                    </p>
                  ...
                  <div className="bg-blue-50 p-3 rounded-xl">
                    ...
                    <p className="font-bold text-gray-800">
                      {confirmedDining.size ||
                        `${confirmedDining.participants?.length || 0}人`}
                    </p>
                  </div>
                </div>
                  ...
                  {confirmedDining.isGroup ? (
                    <>
                      <div className="text-xs text-gray-400 mb-3">
                        生成时间: {confirmedDining.timestamp}
                      </div>
                      <button
    ...
    if (myStatus === "active") {
      activeItems.forEach((item) => {
        if (item.type === "event") {
          ...
          const isPlanMatch = checkIsMatch(lunchDetails, {
            food,
            time,
            location,
          });
            ...
            <div className="space-y-8">
              <div>
                ...
                {matchedItems.length > 0 ? (
                  <div className="space-y-4">
                    {matchedItems.map((item) => renderActiveItem(item))}
                  </div>
                ) : (
              ...
              <div>
                <div className="flex items-center gap-2 mb-3 px-2">
                  ...
                  <h2 className="font-bold text-gray-500"> 正在觅食</h2>
                ...
                {otherItems.length > 0 ? (
                  <div className="space-y-4">
                    {otherItems.map((item) => renderActiveItem(item))}
                  </div>
                ) : (
                  <div className="bg-white/50 border-2 border-dashed border-gray-200 rounded-2xl p-6 flex flex-col items-center justify-center text-center text-gray-400 gap-2">
          ...
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-5 text-white shadow-lg mb-2">
            <div className="flex items-center gap-4">
              <div
                ...
                } flex items-center justify-center text-2xl font-bold border-4 border-white/20 shadow-inner`}
              >
          ...
          {friends.map((friend) => (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div
                  className={`w-10 h-10 shrink-0 rounded-full ${friend.avatarColor} flex items-center justify-center text-white font-bold text-sm`}
                >
                  {friend.nickname[0]}
                </div>
                <div className="flex-1 min-w-0">
              ...
              <div className="space-y-3 mb-4">
                {friendRequests.map((req) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-8 h-8 rounded-full ${req.avatarColor} flex items-center justify-center text-white text-xs`}
                      >
                        {req.nickname[0]}
                      </div>
                      <div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg font-bold"
                    >
                      接受
                    </button>
                  </div>
      ...
      <div className="w-full max-w-md bg-white min-h-[100dvh] relative shadow-2xl flex flex-col">
              ...
              <div className="bg-white w-full max-w-sm rounded-2xl p-6 relative z-10 animate-bounce-in text-center">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 bg-gray-900 text-white rounded-xl font-medium shadow-lg active:scale-95 transition-transform"
                  >
                    确定约！
                  </button>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 relative z-10 animate-bounce-in text-center border-4 border-orange-200">
              ...
              <p className="text-gray-500 mb-6 text-sm">
                <span className="font-bold text-gray-800">我</span>{" "}
                想要和你一起去吃
                <br />
                <span className="font-bold text-orange-500">
                  {friendToDate.lunchPlan
                    ? friendToDate.lunchPlan.food
                    : lunchDetails.food || "随便"}
                </span>
              </p>
              <div className="flex gap-3">
                <button
                  onClick={handlePartnerDecline}
                  className="flex-1 py-4 bg-gray-100 text-gray-500 rounded-2xl font-bold text-lg shadow-sm active:scale-95 transition-transform"
                >
                  暂不回应
                </button>
                <button
                  onClick={handlePartnerAccept}
                  className="flex-1 py-4 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-2xl font-bold text-lg shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2"
                >
        ...
        {showCancelDiningModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            <div
              className="absolute inset-0 bg-black/40 backdrop-blur-sm"
              onClick={() => setShowCancelDiningModal(false)}
            ></div>
            <div className="bg-white w-full max-w-sm rounded-2xl p-6 relative z-10 animate-bounce-in">
